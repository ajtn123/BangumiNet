...
public class Tag(Subjects sub) : ITag
{
    ...
    public int? Count { get; set; } = Common.NumberToInt(sub.AdditionalData.TryGet("count"));
    /// <summary><c>total_cont</c>，好像总是为零。</summary>
    public int? TotalCount { get; set; } = Common.NumberToInt(sub.AdditionalData.TryGet("total_cont"));
}
